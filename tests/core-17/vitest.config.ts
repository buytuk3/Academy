...
/**
 * CORE-17 — Platform Integration & Runtime Verification.
 * Runs against REAL PostgreSQL + REAL Redis (started by the operator).
 * Suites are skipped unless CORE17_RUNTIME=1 so other configs never
 * accidentally depend on live infrastructure.
 */
...
export default defineConfig({
  ...
  test: {
    include: ["tests/core-17/**/*.test.ts"],
    env: {
      // CORE-18/R-008: target DB overridable so the suite can run against a
      // MIGRATION-BUILT database (canonical verification), not only push-built.
      DATABASE_URL: process.env.CORE17_DB_URL ?? "postgres://postgres@127.0.0.1:5432/buytuk_test",
