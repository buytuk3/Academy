...
/**
 * CORE-27 / Batch 2 — EVIDENCE-ONLY tests (no fixes, no product changes).
 * Matrix A (A1–A3): gap-proof tests — EXPECTED TO FAIL against the current
 *   code (R-027-01 / R-027-02). A failure with the predicted reason is the
 *   evidentiary SUCCESS; an unexpected PASS halts the batch.
 * Matrix B (B1–B7): architectural protection tests — MUST ALL PASS.
 * Real PostgreSQL + real Redis + real HTTP. Nothing is mocked.
 */
...
export default defineConfig({
  ...
  test: {
    include: ["tests/core-27/**/*.test.ts"],
    env: {
      DATABASE_URL: process.env.CORE27_DB_URL ?? process.env.DATABASE_URL ?? "",
